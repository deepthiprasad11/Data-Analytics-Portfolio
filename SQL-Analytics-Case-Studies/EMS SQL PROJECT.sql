...
SELECT CONCAT(FirstName ,' ', LastName) AS FullName  , Gender , DateOfBirth , PhoneNumber 
FROM EMPLOYEE E
...
SELECT COUNT(EmployeeID) AS EmployeeCount , DepartmentName
FROM EMPLOYEE E
INNER JOIN DEPARTMENT D
ON E.DepartmentID = D.DepartmentID
...
SELECT FirstName , LastName , ProjectName , JobTitleName , StartDate
FROM EMPLOYEE E
INNER JOIN PROJECTALLOCATION PA
ON E.EmployeeID = PA.EmployeeID
INNER JOIN PROJECT P 
...
RETURN
(
	SELECT FirstName , LastName , DateOfBirth , YEAR(GETDATE()) - YEAR((DateOfBirth)) AS Age
	FROM EMPLOYEE
